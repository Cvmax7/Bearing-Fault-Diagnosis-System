...
class ResidualBlock1D(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        self.conv1 = conv1d_block(in_channels, out_channels, stride=stride)
        self.conv2 = conv1d_block(out_channels, out_channels)
...
class ModelTrainer:
    ...
    def prepare_data(
        self, X_train, y_train, X_val=None, y_val=None, X_test=None, y_test=None
    ):
        """
        准备训练、验证和测试数据
    ...
    def train(self, train_loader, val_loader):
        ...
        for epoch in range(self.num_epochs):
            ...
            print(
                f"Epoch {epoch+1}/{self.num_epochs} - {epoch_time:.1f}s - train_loss: {train_loss:.4f} - train_acc: {train_acc:.4f}",
                end="",
            )
            if val_loader:
    ...
    def export_onnx(self, path=None, sample_input=None):
        ...
        if sample_input is None:
            sample_input = torch.randn(1, self.input_size, self.seq_length).to(
                self.device
            )
